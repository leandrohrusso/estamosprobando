...
| `src/app/(org)/[orgSlug]/*` · `src/app/(auth)/select-organization/*` · `src/components/nav/*` · helpers `requireMembership`/`requireRole`/`getOrgMembers`/`sortMembersPendingFirst` (`src/lib/auth/org.ts`) · actions de miembros | PRP-002 | `tests/e2e/regression/PRP-002-rbac-and-orgs.spec.ts` — RBAC gate (G6) · **no-miembro → 404 en dashboard/members de org ajena (aislamiento · `requireMembership` notFound)** · selector multi-org con submit explícito (G7) · alta pending → vinculación (G8) · CRUD de miembros (add/changeRole/remove con botón "Guardar" · locator `exact` para no colisionar con aria-label superstring · **fila del propio Owner sin controles · anti-lockout**) | E2E | ✅ activo |
