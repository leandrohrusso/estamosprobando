...
- [x] **TASK-002 · Auth + organizaciones + memberships + RLS base** — Supabase Auth (magic link), tablas `organizations` + `memberships`, RLS por `organization_id`, selección de organización, RBAC (Owner/Admin/Staff). _Base del aislamiento multi-tenant (constraint #1)._ → **[PRP-002](../../.claude/PRPs/PRP-002-auth-organizations-memberships-rls.md) COMPLETADO 2026-07-04** (mergeado `--squash` a `main` · `80964f0` · CI remoto 7/7 verde). Entregó: magic link + `/auth/confirm` + `organizations`/`memberships` (enums + índices) + RLS multi-tenant read/write vía helpers `SECURITY DEFINER` (Owner inmutable a nivel RLS) + RPCs idempotentes + onboarding→Owner + selector multi-org + gestión de miembros Owner-only. **DT-001 cerrada.** `/validar`: CSV 55F/1D (DT-003 magic link). Cierre `/entregar`: 2 fixes CI-red in-scope (env secrets Supabase en jobs e2e/sql + polyfill `ws` WebSocket para Node 20). DTs abiertas: DT-003/006/007/008.
